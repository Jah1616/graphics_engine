...
void createCube(Figure3D& figure){
    ...
    faces = {{0,4,2,6}, {4,1,7,2}, {1,5,3,7}, {5,0,6,3}, {6,2,7,3}, {0,5,1,4}};
}
...
void createIcosahedron(Figure3D& figure){
    ...
    points = mem_points;
    faces = {{0,1,2}, {0,2,3}, {0,3,4}, {0,4,5}, {0,5,1}, {1,6,2}, {2,6,7}, {2,7,3}, {3,7,8}, {3,8,4}, {4,8,9},
...
void createDodecahedron(Figure3D& figure){
    ...
    points = mem_points;
    faces = {{0,1,2,3,4}, {0,5,6,7,1}, {1,7,8,9,2}, {2,9,10,11,3}, {3,11,12,13,4}, {4,13,14,5,0},
...
void createCylinder(Figure3D& figure, const unsigned int n, const double h){
    ...
    for (unsigned int i=0 ; i<n ; ++i){
        points.push_back(Vector3D::point(mem_cos[n][i], mem_sin[n][i], 0));
        base.push_back(n-i-1);
    }

    Face top;
    for (unsigned int i=0 ; i<n ; ++i){
        points.push_back(Vector3D::point(mem_cos[n][i], mem_sin[n][i], h));
        top.push_back(n+i);
...
void createSphere(Figure3D& figure, const unsigned int n){
    ...
    points = std::vector(mem_points.begin(), mem_points.begin() + (5 * pow(4, n + 1) - 8));
    for (Vector3D& p : points) p.normalise();
    faces = mem_faces[n];
}
