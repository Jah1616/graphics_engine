...
namespace
{
    ...
    void parse_chances(std::set<char> const& alphabet, std::map<char, double>& chances, stream_parser& parser)
    {
        parser.skip_comments_and_whitespace();
        parser.assertChars("Chances");
        parser.skip_comments_and_whitespace();
        parser.assertChars("=");
        parser.skip_comments_and_whitespace();
        parser.assertChars("{");
        ...
        while (true)
        {
            if (!std::isalpha(c))
                throw LParser::ParserException("Invalid Alphabet character", parser.getLine(), parser.getCol());
            if (alphabet.find(c) == alphabet.end())
                throw LParser::ParserException(std::string("Replacement chance specified for char '") + c + "' which is not part of the alphabet. ", parser.getLine(), parser.getCol());
            if (chances.find(c) != chances.end())
                throw LParser::ParserException(std::string("Double entry '") + c + "' in chances specification ", parser.getLine(), parser.getCol());
            char alphabet_char = c;
            parser.skip_comments_and_whitespace();
            parser.assertChars("->");
            parser.skip_comments_and_whitespace();
            double chance = parser.readDouble();
            chances[alphabet_char] = chance;
            parser.skip_comments_and_whitespace();
            c = parser.getChar();
            if (c == '}')
                break;
...
double LParser::LSystem::get_chance(char c) const
{
...
std::istream& LParser::operator>>(std::istream& in, LParser::LSystem2D& system)
{
    ...
    // Check if "Chances" section exists (SOURCE: ChatGPT)
    std::streampos chancesSectionStart = in.tellg();
    parser.skip_comments_and_whitespace();
    std::string nextToken;
    in >> nextToken;
    if (nextToken == "Chances") {
        // Reset stream position to the beginning of "Chances" section
        in.seekg(chancesSectionStart);
        parse_chances(system.alphabet, system.chances, parser);
    } else {
        // "Chances" section doesn't exist, so reset the stream position
        // to before the "Chances" section and continue parsing
        in.seekg(chancesSectionStart);
    }
